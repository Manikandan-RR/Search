...
export function HomePage() {
    ...
    const filteredProducts = productList.filter((product, i) => product.title.toLowerCase().includes(search.toLowerCase()))

    return (
        <>
            <div className="container-fluid mt-4">
                <h1>The Selected Cart IS: {cart}</h1>
                {/* <input type="text" className="form-control w-25 mb-3" placeholder="Search products..." value={search} onChange={(e) => setSearch(e.target.value)} /> */}
                <div className="row g-4">
                    {filteredProducts.map((product, i) =>
                        <div className="col-3" key={product.id}>
                            <ProductCard title={product.title} price={product.price} image={product.image} description={product.description} id={product.id} rating={product.rating} addCart={handleCart} />
                        </div>
